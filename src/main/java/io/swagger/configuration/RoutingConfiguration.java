...
public class RoutingConfiguration {
    ...
    {
        Optional<RpcRouter> router = RpcRouterBuilder.instance(Actor.getInstance(BuiltInActorType.EMAIL_SENDER.verbLabel()).get())
    			.addServiceSupport(ServiceType.SENDER, Actor.getInstance(BuiltInActorType.DB_CONNECTOR.verbLabel()).get())
    			.setNumberOfPublishers(numOfPublishers)
    			.setNumberOfConsumers(numOfConsumers)
    			.setRoutingKeyPrefix("sender")
    			.build();
